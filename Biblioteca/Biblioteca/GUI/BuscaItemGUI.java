...
public class BuscaItemGUI {
		...
		if (JButton == null) {
			...
			JButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					Erro[0]="";
					if (jTextoField.getText().length() == 0){
						OK = false;
					}
					int chosedTipo =  (Integer)choice.getSelectedIndex();
					if (select!=9){
						//OK=validador.validaCampoTexto(jTextoField, "O campo selecionado", Erro);
						if (OK){
							DadosTabela dados = controlador.BuscarNome(
													chosedTipo,select,
													jTextoField.getText()
													);
							
							chamadoPor.MostraTabelaResultados(dados,
									"Resultados da busca de Itens com o texto "+jTextoField.getText(),
									"Item", null);
						...
						if (OK){
							...
							/*DadosTabela dados = controlador.BuscarNumero(
									choice.getSelectedIndex()+1,
									Integer.parseInt(jTextoField.getText()));
							 */
							chamadoPor.MostraTabelaResultados(dados,
									"Resultados da busca de Itens com o código "+jTextoField.getText(),
									"Item", null);
						}
						else{
							validador.dialogoErroPreenchimento(Erro[0]);
	...
	private JComboBox getChoice() {
		if (choice == null) {
			choice = controlador.getTiposComboSrc();
	...
	public BuscaItemGUI(BibliotecaGUI ChamadoPor){
