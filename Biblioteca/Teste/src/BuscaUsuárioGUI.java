package src;
/*Resolver o problema das checkboxes e
 * criar função do botão OK
 */
			...
			buttonBuscar.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (select==9){
						...
						ControladorUsuário.BuscarNome(jTextoField.getText());
						}
					else {
						OK = validador.validaCampoNumero(jTextoField, "O campo selecionado", Erro);
						if(OK){
							ControladorUsuário.BuscarNumero	(select,
																Integer.parseInt(
																			jTextoField.getText()
																					)
																);
